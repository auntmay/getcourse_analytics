...
from models import Order, Client, Expens
...
def delete_orders_data(current_user_id):
    ...
    Expens.query.filter(Expens.user_id == current_user_id).delete()
    db_session.commit()
...
def create_result_df(current_user_id):
    orders_sql_data = get_orders_data(current_user_id)
